...
type Resolver struct {
	...
	// Test mocks
	testQueryHook    func(name string, nameserver netip.Addr, protocol string, qtype dns.Type) (*dns.Msg, error)
...
// returned records.
func (r *Resolver) Resolve(ctx context.Context, name string) ([]netip.Addr, time.Duration, error) {
	qstate := r.newState()

	r.logf("querying IPv4 addresses for: %q", name)
	addrs4, minTTL4, err4 := r.resolveRecursiveFromRoot(ctx, qstate, 0, name, qtypeA)
	...
	if !r.NoIPv6 {
		...
		addrs6, minTTL6, err6 = r.resolveRecursiveFromRoot(ctx, qstate, 0, name, qtypeAAAA)
	}
...
func (r *Resolver) resolveRecursive(
	...
	for _, answer := range resp.Answer {
		if crec, ok := answer.(*dns.CNAME); ok {
			cnames = append(cnames, crec.Target)
			continue
		}
		...
		if qtype == qtypeA {
			rec, ok := answer.(*dns.A)
			if !ok {
				r.logf("bad type returned for A query: %T", answer)
				continue
			}
			ip, ok := netip.AddrFromSlice(rec.A)
			if !ok || !ip.Is4() {
				r.logf("unexpected bad IPv4 addr")
				continue
			}
			answers = append(answers, ip)
			minTTL = min(minTTL, int(rec.Hdr.Ttl))

		} else if qtype == qtypeAAAA {
			rec, ok := answer.(*dns.AAAA)
			if !ok {
				r.logf("bad type returned for AAAA query: %T", answer)
				continue
			}
			ip, ok := netip.AddrFromSlice(rec.AAAA)
			if !ok || !ip.Is6() {
				r.logf("unexpected bad IPv6 addr")
				continue
			}
			answers = append(answers, ip)
			minTTL = min(minTTL, int(rec.Hdr.Ttl))
	...
	for _, rr := range resp.Ns {
		switch v := rr.(type) {
		case *dns.NS:
			authorities = append(authorities, v.Ns)
		}
	}
	...
	for _, rr := range resp.Extra {
		...
		case *dns.A:
			...
			if !ok || !ip.Is4() {
				r.logf("unexpected bad Extra IPv4 addr")
				continue
			}
			glueRecords[v.Hdr.Name] = append(glueRecords[v.Hdr.Name], ip)
		case *dns.AAAA:
			ip, ok := netip.AddrFromSlice(v.AAAA)
			if !ok || !ip.Is6() {
				r.logf("unexpected bad Extra IPv6 addr")
				continue
			}
			glueRecords[v.Hdr.Name] = append(glueRecords[v.Hdr.Name], ip)
		default:
	...
	var authoritiesGlue, authoritiesNoGlue []string
	for _, authority := range authorities {
		if len(glueRecords[authority]) > 0 {
			authoritiesGlue = append(authoritiesGlue, authority)
		} else {
			authoritiesNoGlue = append(authoritiesNoGlue, authority)
		}
	}
...
func (r *Resolver) queryNameserverProto(
	...
	//r.depthlogf(depth,"response from %s: %+v", nameserverStr, resp)
	return resp, nil
}
